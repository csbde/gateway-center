...
- [X] T090 错误文案审计：逐条 VALIDATION_FAILED/PIPELINE_BLOCKED 确认含资源定位 + hint 修复建议（NFR-USE-01），修 backend/internal/api/httperr/ 与前端错误映射
- [X] T091 [P] 覆盖率门槛：管线相关包 ≥80% 入 backend/Makefile test-all（unit+contract+integration+e2e 串行）（R17/宪法合规审查）
- [X] T092 宪法符合性人工检查：执行 quickstart.md §5 表全部项（无反向 YAML 解析 grep、写路径仅 dynamic/、TestNoPipelineBypass、快照自包含用例、审计 revoke）并记录结果
